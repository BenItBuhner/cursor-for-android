...
- Conversation view: transcript (`/v0/agents/{id}/conversation`), run footers ("Worked 3m 5s" + branch / PR pills), a pull-request button in the header beside the menu once the agent has opened one, live SSE streaming of the active run, subagent cards and markdown rendering; follow-ups with image attachments (`prompt.images`), and stop. A stream that drops — a network change, the agent's machine waking up after a follow-up (`stream_unavailable`), a server-side `upstream_error` — is a connection problem, not the run's: the trace stays put, the working row says "Reconnecting…", and the stream is resumed with `Last-Event-ID` and backoff for as long as the run record says the run is going; a run whose stream is gone for good is settled from the record instead. Everything the agent does between two replies — its reasoning and tool calls, in the order they happened — sits behind one "Explored N files, M searches · thought for Ns" row ("Exploring …" with a spinner while it is still going) that expands to the interleaved trace: each thought as prose, each run of tool calls as a card. Finished runs replay their retained event stream, so the same thinking / tool / subagent trace is there to dig into after the fact; a trace seen whole — followed live to its `result` (on screen or by the live notification) or replayed — is kept on disk from then on, and a run that ends while the screen is away, or whose stream broke, is replayed from its first event rather than left as the fragment the screen had. Runs whose log expired (`410 stream_expired`) before it was ever seen keep the text-only transcript.
