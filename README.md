...
- Conversation view: transcript (`/v0/agents/{id}/conversation`), run footers ("Worked 3m 5s" + branch / PR pills), live SSE streaming of the active run with thinking, "Explored N files, M searches" tool rows, subagent cards and markdown rendering; follow-ups with image attachments (`prompt.images`), and stop. A stream that drops — a network change, the agent's machine waking up after a follow-up (`stream_unavailable`), a server-side `upstream_error` — is a connection problem, not the run's: the trace stays put, the working row says "Reconnecting…", and the stream is resumed with `Last-Event-ID` and backoff for as long as the run record says the run is going; a run whose stream is gone for good is settled from the record instead. Finished runs replay their retained event stream, so the same thinking / tool / subagent trace is there to dig into after the fact; runs past the API's retention window (`410 stream_expired`) keep the text-only transcript.
