...
- New Chat pane as home: repository / branch / environment selectors, the composer (the "+" menu, model picker with plan mode and auto-PR, send), then the recent chats with preview cards. The model picker lists each model from `GET /v1/models` once; a model's parameters — effort, speed, context window — unfold under it as pickers (a toggle for an on/off parameter, a row of choices otherwise), and every choice resolves to a variant the API lists, so the request never carries a `model.params` combination the model does not accept.
