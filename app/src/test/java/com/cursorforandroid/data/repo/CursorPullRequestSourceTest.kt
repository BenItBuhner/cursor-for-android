...
class CursorPullRequestSourceTest {
    ...
    fun `an answer is found, no answer is unreadable`() = runBlocking<Unit> {
        assertThat(source.lookup(url)).isEqualTo(PullRequestLookup.Found(PullRequestState.Open))
        answer = { null }
        assertThat(source.lookup(url)).isEqualTo(PullRequestLookup.Unreadable)
    }
    ...
    fun `a refusal for one pull request is unreadable, a server or network failure is a failure`() = runBlocking<Unit> {
        ...
        assertThat(source.lookup(url)).isEqualTo(PullRequestLookup.Failed)
    ...
    fun `a session failure retrying cannot fix halts the source until it is reset`() = runBlocking<Unit> {
        ...
        source.reset()
        assertThat(source.lookup(url)).isEqualTo(PullRequestLookup.Found(PullRequestState.Merged))
    }
    ...
    fun `a session failure that may pass does not halt it`() = runBlocking<Unit> {
        ...
        assertThat(source.lookup(url)).isEqualTo(PullRequestLookup.Found(PullRequestState.Closed))
    }
}
