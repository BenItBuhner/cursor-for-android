...
class AccountSimulationTest {
    ...
    private fun buildAccount() {
        ...
        projects.forEach { p ->
            ...
            repeat(p.members) { index ->
                ...
                // Codex Meter's first worker runs: the status scan brings it, bare, from far beyond the loaded pages.
                val running = (!p.archived && random.nextInt(10) == 0) || id == "bc-meter-m1"
                when {
    ...
    private fun agents() = AgentRepository(
        ...
        // Every agent the scan names lands in one pass here (the app takes a dozen per poll, and polls).
        maxMaterializedRunning = 200,
        recordOf = { id -> if (extended) account.record(id) else null },
    )

    /** As the sidebar's view model wires it: watching the list for parents named but not loaded, fetched by id. */
    private fun projectsOf(agents: AgentRepository) = ProjectRepository(session, agents, account, scope = scope, pollIntervalMs = 60_000, capabilities = capabilities, retryDelaysMs = listOf(50L, 50L, 50L)).also { it.watchList() }
    ...
    private fun sections(agents: AgentRepository, projects: ProjectRepository?, prefs: ListPreferences = ListPreferences(), rows: List<Agent> = agents.state.value.agents) = runBlocking {
        ...
        AgentListOrganizer.organize(
            rows, prefs, local, nowMillis = 1_800_000_100_000L, zone = ZoneOffset.UTC,
    ...
    private fun assertInvariants(label: String, agents: AgentRepository, projects: ProjectRepository?) {
        // One snapshot of the rows for everything below: the repository keeps fetching records and rows meanwhile.
        val rows = agents.state.value.agents
        val sections = sections(agents, projects, rows = rows)
        ...
        val loadedOwn = rows.filter { byId[it.id]?.own == true }
        ...
        row("bc-market")?.let { assertWithMessage("$label: Market Opportunities is a chat of the account's own").that(it.scope).isEqualTo(AgentScope.PRIMARY) }
        row("bc-newflag")?.let { assertWithMessage("$label: startedAsNewProject alone flags nothing").that(it.scope).isEqualTo(AgentScope.PRIMARY) }
        row("bc-meter")?.let { assertWithMessage("$label: a coordinator without project_metadata is a chat of the account's own (kf)").that(it.scope).isEqualTo(AgentScope.PRIMARY) }
        // `project_metadata: {}` is the desktop's flag: a Project with no members, drawn as the desktop draws it.
        row("bc-emptymeta")?.let { assertWithMessage("$label: an empty project_metadata is the desktop's flag").that(it.scope).isEqualTo(AgentScope.PROJECT_ROOT) }
        ...
        row("bc-meta-1")?.let { assertWithMessage("$label: a meta agent chat is a chat of the account's own").that(it.scope).isEqualTo(AgentScope.PRIMARY) }
        row("bc-spawner-1")?.let { assertWithMessage("$label: a chat with subagents is a chat of the account's own").that(it.scope).isEqualTo(AgentScope.PRIMARY) }
        ...
        if (spawner != null && row("bc-spawner-1-sub1") != null) {
        ...
        assertDesktopTopLevel(label, rows, sections)
    }
     ...
     */
    private fun assertDesktopTopLevel(label: String, rows: List<Agent>, sections: List<com.cursorforandroid.domain.AgentSection>) {
        val loaded = rows.associateBy { it.id }
    ...
    fun `an install upgraded from 0_3_4 lists every Project on the first launch, with the session minting late and a page failing`() = runBlocking<Unit> {
        ...
        // Codex Meter is an ordinary row with its worker under it — the desktop's row-with-children for a
        // coordinator the record does not flag: the running worker came bare from the status scan, its record by
        // id named the manager, and the manager's row was fetched by id to head it (Bennett's 0.3.8 rows).
        awaitUntil("Codex Meter and its running worker to be loaded") { agents.agent("bc-meter-m1")?.record != null && agents.agent("bc-meter") != null }
        ...
        assertThat(meter.children.map { it.agent.id }).contains("bc-meter-m1")
        assertThat(meter.agent.isProjectRoot).isFalse()
    ...
    /**
     * What Bennett saw on 0.3.5, reproduced: nothing of the account list read at launch — the pin repository was
     * built by the first scroll to the sidebar's end, half an hour in — and when it was, the discovery pass read one
     * page and called itself complete. Every row is asked for its own record by id now, and a parent a row names is
     * fetched by id, so the Projects the page's rows and the running workers lead to are known before any list round
     * — the way the desktop knows every row's record — and the archived Project with no running worker still takes
     * the discovery pass over the whole list.
     */
    @Test
    fun `0_3_5 as it ran - no account list until the pin repository is touched, and the discovery pass reaching one page`() = runBlocking<Unit> {
        account.pagesServed = 1
        val agents = agents()
        val projects = projectsOf(agents)
        agents.refresh()
        // Launch: the public list's page and the running set, each row with its own record, each parent named fetched
        // by id: the Projects those lead to and no other — never Codex Meter, which the record does not flag.
        awaitUntil("the page's rows to have their records") { agents.state.value.agents.all { it.record != null } }
        awaitUntil("the running workers' Projects to be fetched") { setOf("bc-revenue", "bc-shipyard", "bc-noetic", "bc-murmur").all { agents.agent(it) != null } }
        val atLaunch = sections(agents, projects).firstOrNull { it.key == AgentListOrganizer.PROJECTS_KEY }?.rows.orEmpty().map { it.agent.id }
        assertThat(atLaunch).containsNoneOf("bc-retro", "bc-meter")
        assertThat(atLaunch).containsAtLeast("bc-revenue", "bc-shipyard", "bc-noetic", "bc-murmur")
        assertThat(account.listCalls.get()).isEqualTo(0)
        assertThat(agents.knownRoots.value.map { it.id }).doesNotContain("bc-retro")
        ...
        awaitUntil("the roots the page named to be fetched") { setOf("bc-revenue", "bc-shipyard", "bc-mobile", "bc-jobs", "bc-emptymeta").all { agents.agent(it) != null } }
        val group = sections(agents, projects).firstOrNull { it.key == AgentListOrganizer.PROJECTS_KEY }?.rows.orEmpty()
        assertWithMessage("the Projects among the newest 200 records are in the group").that(group.map { it.agent.name })
            .containsAtLeast("Revenue Scaling Pipeline", "Shipyard", "Cursor for Android", "Job & Bounty Research", "Empty project metadata")
        assertWithMessage("the archived Project beyond the page is not known yet").that(agents.knownRoots.value.map { it.id }).doesNotContain("bc-retro")
        assertThat(projects.lastRootScan.value!!.pagesRead).isEqualTo(1)

        // The same account with the pages reachable: the next pass names every flagged Project, the archived one too.
    ...
    fun `the live count is every running agent while the Projects' chats keep their cards off, in both modes`() = runBlocking<Unit> {
        ...
        // Default mode can read no record: what it knows of the Projects' chats is what Extended mode wrote to the
        // disk — so the switch waits for every loaded row's record, and for the disk to hold the rows.
        awaitUntil("every loaded row to have its record") { agents.state.value.agents.all { it.record != null } }
        awaitUntil("the rows on disk with their records") { cache.read()?.value?.let { rows -> rows.isNotEmpty() && rows.all { it.record != null } } == true }
        extended = false
