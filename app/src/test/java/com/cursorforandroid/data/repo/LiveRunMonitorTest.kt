...
class LiveRunMonitorTest {
    ...
    fun `conversation replays finished runs, keeps text past the retention cutoff and streams follow-ups`() = runBlocking {
        ...
        assertThat(types.drop(12)).containsExactly("UserMessage", "ThinkingBlock", "ToolActivity", "AssistantMessage", "RunFooter").inOrder()
        assertThat((state.items[12] as UserMessage).text).isEqualTo("Prompt 5")
        assertThat((state.items[15] as AssistantMessage).markdown).isEqualTo("Reply 5")
        assertThat((state.items[16] as RunFooter).durationMs).isEqualTo(42_000L)
        ...
        assertThat(done.items.map { it::class.simpleName }.takeLast(4)).containsExactly("UserMessage", "ToolActivity", "AssistantMessage", "RunFooter").inOrder()
    ...
    fun `a follow-up on an agent without a transcript still lands after its history`() = runBlocking {
        ...
        assertThat(items.map { it::class.simpleName }).containsExactly(
            ...
            "UserMessage", "ToolActivity", "AssistantMessage", "RunFooter",
        ).inOrder()
