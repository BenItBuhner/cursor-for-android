 ...
 * flies into the first bubble of the chat it starts, the chat fading in over the pane rather than sliding in.
 *
...
class SendAnimationFlowTest {
    ...
    /** What a flight went through, frame by frame, with the frames written out for the demo when asked. */
    private class Flown(val phases: List<SendFlight.Phase>, val flight: SendFlight, val midway: Boolean)
    ...
    private fun sendAndFly(composerTag: String, text: String, demoName: String): Flown {
        ...
        compose.mainClock.autoAdvance = true
        compose.waitForIdle()
        return Flown(phases.distinct(), flight, midway)
    }
