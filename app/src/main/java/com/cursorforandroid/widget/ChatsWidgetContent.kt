 ...
 * working, the blue unread dot, the red error dot, the purple branch glyph for a read chat that pushed a branch,
 * the archive box, and nothing for a plain read chat.
 */
@Composable
private fun StateGlyph(row: AgentRow, palette: WidgetPalette) {
    ...
    when (row.indicator) {
        ...
        AgentIndicator.Read -> if (row.agent.hasBranch) Image(ImageProvider(R.drawable.widget_git_branch), "Branch", slot, colorFilter = ColorFilter.tint(palette.branchGlyph))
